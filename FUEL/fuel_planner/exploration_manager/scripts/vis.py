...
class MultiUAVVisualizer:
    def __init__(self, num_uavs=1):
        ...
        for i in range(0, num_uavs + 1):
            topic_name = f'/mavros/vision_pose/pose'
            self.paths[i] = Path()
            self.paths[i].header.frame_id = "world"

            rospy.Subscriber(topic_name, PoseStamped, self.pose_callback, i)

            self.path_pubs[i] = rospy.Publisher(f'/uav_path', Path, queue_size=10)
    ...
    def publish_uav_icon(self, uav_id, pose):
        ...
        icon_marker.mesh_resource = "package://fuel_exploration_manager/scripts/meshes/hummingbird.mesh"  # 模型文件路径
...
if __name__ == '__main__':
    try:
        visualizer = MultiUAVVisualizer(num_uavs=1)
        rospy.spin()
